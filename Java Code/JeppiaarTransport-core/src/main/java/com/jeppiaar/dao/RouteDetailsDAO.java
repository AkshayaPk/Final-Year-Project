...
public class RouteDetailsDAO {
  ...
  */
	...
	{
		SimpleJdbcCall call = new SimpleJdbcCall(jdbcTemplate).withProcedureName("PR_UPDATE_ROUTE_NAME").declareParameters(
				new SqlParameter("i_busno", Types.INTEGER),new SqlParameter("i_routename",Types.VARCHAR),new SqlParameter("errmsg", Types.VARCHAR));
		call.setAccessCallParameterMetaData(false);
		SqlParameterSource in = new MapSqlParameterSource().addValue("i_busno", busno).addValue("i_routename",routeName).addValue("errmsg",errmsg);
		Map<String,Object> execute=call.execute(in);
		return (String) execute.get(errmsg);
	...
	{
		SimpleJdbcCall call = new SimpleJdbcCall(jdbcTemplate).withProcedureName("PR_UPDATE_BUS_NO").declareParameters(
				new SqlParameter("i_routename", Types.VARCHAR),new SqlParameter("i_busno",Types.INTEGER),new SqlOutParameter("errmsg", Types.VARCHAR));
		call.setAccessCallParameterMetaData(false);
		SqlParameterSource in = new MapSqlParameterSource().addValue("i_routename", routeName).addValue("i_busno",busNo).addValue("errmsg",errmsg);
		Map<String,Object> execute=call.execute(in);
		return (String) execute.get(errmsg);
				
	}
	public String updateRouteIntermediary(Integer busNo,String ip1,String ip2,String ip3,String ip4,String ip5,String ip6,String errmsg)
	{
		SimpleJdbcCall call = new SimpleJdbcCall(jdbcTemplate).withProcedureName("PR_UPDATE_INTERMEDIARY").declareParameters(
			new SqlParameter("i_busno",Types.INTEGER),new SqlParameter("i_ip1",Types.VARCHAR),new SqlParameter("i_ip2",Types.VARCHAR)
			,new SqlParameter("i_ip3",Types.VARCHAR),new SqlParameter("i_ip4",Types.VARCHAR),new SqlParameter("i_ip5",Types.VARCHAR),new SqlParameter("i_ip6",Types.VARCHAR),new SqlOutParameter("errmsg", Types.VARCHAR));
		call.setAccessCallParameterMetaData(false);
		SqlParameterSource in = new MapSqlParameterSource().addValue("i_busno",busNo).addValue("i_ip1", ip1).addValue("i_ip2", ip2)
				.addValue("i_ip3", ip3).addValue("i_ip4", ip4).addValue("i_ip5", ip5).addValue("i_ip6", ip6).addValue("errmsg",errmsg);
		Map<String,Object> execute=call.execute(in);
		return (String) execute.get(errmsg);
